...
(véase la Figura~\ref{fig::lit_timeline}).

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/fig_lit_timeline.png}
\caption{La literatura académica sobre Polymarket surgida en la primera mitad de 2026 y la
posición de este trabajo. Los cinco estudios describen la plataforma (elecciones, arbitraje,
flujo, archivos on-chain o tick-level); este trabajo se distingue por añadir el libro L2
sincronizado con referencias externas y un contrato de latencia, lo que habilita la
evaluación económica de la ejecución que ninguno aborda.}
...
\emph{taker} de latencia que este trabajo confirma por otra vía. Ese hallazgo acota
también la alternativa práctica más visible fuera de la literatura académica: los
\emph{bots} de arbitraje de código abierto que circulan en repositorios públicos,
que cruzan desajustes elementales (los dos lados de un mercado sumando menos de uno,
o el mismo suceso cotizado distinto en dos mercados) sin modelo alguno. De ellos este
trabajo aprovecha la lección de ingeniería (la conexión continua a la API y la
contabilidad de costes por operación son la mitad del problema) y se diferencia en el
objetivo: no ejecutar la anomalía obvia, que la evidencia muestra escasa y efímera,
sino medir si existe señal explotable bajo costes y latencia reales.
...
La Figura~\ref{fig::deeplob_esquema} ilustra la idea que da nombre a la ``lectura visual''
del libro: el estado del mercado en cada instante se apila como una columna (precios y
volúmenes de los mejores niveles de compra y de venta) y la sucesión de instantes forma
una matriz que la convolución recorre igual que recorrería una imagen, buscando motivos
locales (desequilibrios entre lados, escalones de profundidad) que la recurrencia integra
...
\begin{tikzpicture}[
  font=\small,
  caja/.style={draw, rounded corners=2pt, minimum height=9mm, align=center},
  flecha/.style={-{Stealth[length=2.5mm]}, thick}
]
% matriz del libro
\foreach \x in {0,...,7} {
  \foreach \y in {0,...,4} {
    \pgfmathsetmacro{\g}{85 - mod (\x*13+\y*29,45)}
    \fill[black!\g!white] (\x*0.42, \y*0.42) rectangle ++(0.38, 0.38);
  }
}
\draw[thick] (-0.15,-0.05) rectangle (3.35, 2.15);
\node[below, align=center] at (1.6,-0.15) {tiempo $\rightarrow$\\[-2pt]\scriptsize (fotografías cada 2\,s)};
\node[rotate=90, above, align=center] at (-0.35,1.05) {niveles del libro};
% flechas y bloques
\coordinate (mat) at (3.35,1.05);
\node[caja, right=9mm of mat] (conv) {convoluciones\\\scriptsize estructura espacial};
\node[caja, right=7mm of conv] (rec) {recurrencia /\\atención\\\scriptsize dinámica temporal};
\node[caja, right=7mm of rec] (out) {señal\\\scriptsize sube / baja / nada};
\draw[flecha] (3.45,1.05) -- (conv);
\draw[flecha] (conv) -- (rec);
\draw[flecha] (rec) -- (out);
\end{tikzpicture}
\caption[La ``lectura visual'' del libro de órdenes]{Esquema de la familia DeepLOB: el libro
se representa como una matriz niveles$\times$tiempo (cada celda, un precio o volumen) que
los módulos convolucionales procesan como una imagen antes de que la parte recurrente (o
de atención) integre la evolución temporal. El BookEncoder del Capítulo~\ref{ch::metodos}
es la adaptación de esta idea al libro de Polymarket.}
\label{fig::deeplob_esquema}
\end{figure}
...
un fundacional \emph{del tipo de dato que ganó}, el tabular, y no solo del tipo de
dato aparente, la serie temporal?), cuyos resultados se presentan en la
Sección~\ref{sec::tabpfn}.
...
La Figura~\ref{fig::model_map} resume las familias evaluadas y su relación con el
especialista tabular que este trabajo acaba eligiendo.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/fig_model_map.png}
\caption{Familias de modelos consideradas (fundacionales de series, auto-supervisados y el
fundacional tabular) y su papel como peldaños de la escalera comparativa frente al
especialista tabular, que es el vehículo finalmente elegido de la señal. Las estrellas
marcan los modelos efectivamente evaluados en este trabajo (MOMENT y TabPFN).}
\label{fig::model_map}
\end{figure}
...
de Bitcoin, de modo que cualquier lector puede verificar la fecha del compromiso frente a
las fechas de la ventana de evaluación, sin confiar en el autor ni en un tercero. Una
precisión que conviene dejar escrita: el sello garantiza que el compromiso precede a la
\emph{observación} de los datos, no a su existencia (al sellarse el protocolo general ya
se habían capturado los primeros días de su ventana). Para ese tramo inicial la garantía
es la declaración del propio documento sellado, que fija el compromiso ``antes de mirar
cualquier dato'', y la frontera entre la garantía criptográfica y la procedimental se
declara aquí en lugar de dejarla implícita. La
combinación (candidato congelado con hashes verificables, ventana de evaluación
prospectiva intocada hasta un corte fijado de antemano, y reporte de todas las variantes
comprometidas, favorables o no) es, hasta donde alcanza la revisión de este capítulo,
...
La Tabla~\ref{tab::familias} condensa el balance de las familias revisadas: qué fortaleza
motiva incluir cada una, qué debilidad documenta la literatura o anticipa este corpus, y
qué se adopta de ella en el trabajo.

\begin{table}[htbp]
\centering
\caption[Balance de las familias de modelos revisadas]{Puntos fuertes y débiles de cada
familia revisada y qué adopta este trabajo de cada una.}
\label{tab::familias}
\small
\begin{tabular}{>{\raggedright\arraybackslash}p{0.21\textwidth}>{\raggedright\arraybackslash}p{0.24\textwidth}>{\raggedright\arraybackslash}p{0.25\textwidth}>{\raggedright\arraybackslash}p{0.21\textwidth}}
\toprule
\textbf{Familia} & \textbf{Punto fuerte} & \textbf{Punto débil} & \textbf{Qué se adopta} \\
\midrule
CNN/LSTM sobre el libro (DeepLOB, TLOB) &
Estructura espacial del libro; F1 $\sim$83\,\% en FI-2010 &
Exige subyacente en movimiento continuo; frágil ante coste y latencia &
Representación matriz niveles$\times$tiempo (BookEncoder) \\
\addlinespace
Fundacionales de series (MOMENT, Chronos, TimesFM) &
\emph{Zero-shot} y ajuste barato; contexto largo &
No baten a baselines simples en régimen casi inmóvil; contexto de 512 no cabe en mercados
cortos &
MOMENT como peldaño profundo de la escalera \\
\addlinespace
Fundacional tabular (TabPFN) &
Ordenación \emph{zero-shot} competitiva con un tercio de los datos &
Tope de contexto; no mejora la cabeza de decisión &
Pregunta simétrica de la escalera (Sección~\ref{sec::tabpfn}) \\
\addlinespace
Auto-supervisión (TS2Vec, TCN propio) &
Aprovecha datos sin etiquetar; regulariza &
Sin etiqueta económica, la representación no encuentra señal que no exista &
Encoder TCN auto-supervisado como peldaño \\
\addlinespace
Tabulares con regularización (HGB) &
Varianza baja con pocos casos; robustos a escala y ruido &
Sesgo alto; no capturan estructura secuencial fina &
Especialista de dos cabezas (el modelo final) \\
\bottomrule
\end{tabular}
\end{table}
