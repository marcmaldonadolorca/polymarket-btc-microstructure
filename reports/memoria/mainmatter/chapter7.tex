...
\label{sec::recolector_no_publica}
...
captura) no se publica por tamaño. Tampoco existe un script de descarga que lo sustituya, y
el motivo es de naturaleza de los datos: el corpus no procede de una descarga, sino de una
captura en tiempo real. La API del libro de órdenes que consume el recolector devuelve la
fotografía presente del libro, no su historia, de modo que la serie de libros solo puede
obtenerse observándola en el momento; las referencias externas y el flujo de operaciones sí
tienen histórico, pero sin el libro alineado no reconstruyen el corpus. Quien quiera un
corpus equivalente tiene que capturar una ventana nueva con un recolector como el descrito
en la Sección~\ref{sec::captura}, y esa ventana será otra. En su lugar se incluyen el esquema SQL, un ledger anonimizado suficiente
para reproducir las cifras finales y el código del \emph{pipeline}. Esto permite una
